...
**建议补充的截图**（把图片放进 `docs/images/` 后按下面的文件名命名即可自动对应）：

| 文件名 | 页面 | 截图要点 |
| --- | --- | --- |
| `02-dashboard.png` | 数据看板 | 4 个指标卡 + 订单/成交额趋势图 + 热销商品榜 |
| `03-product-list.png` | 商品列表 | 多条件筛选、库存与销量列、低库存红色告警 |
| `04-product-form.png` | 商品编辑弹窗 | 分类级联、价格校验、初始库存初始化 |
| `05-category.png` | 分类管理 | 两级分类树形表格 |
| `06-inventory.png` | 库存管理 | 可用/锁定库存、入库与盘点弹窗、库存流水抽屉 |
| `07-order-list.png` | 订单列表 | 状态标签、支付/取消/发货/完成操作按钮 |
| `08-order-detail.png` | 订单详情 | 商品快照明细 + 状态流转时间线 |
| `09-user-role.png` | 用户与角色 | 角色分配弹窗 |
| `10-operation-log.png` | 操作日志 | AOP 审计记录、耗时高亮 |
| `11-swagger.png` | 接口文档 | OpenAPI 在线调试页面（dev 环境） |
...
- **单 Maven 模块而非多模块**：单人迭代下多模块只增加构建与依赖管理成本；
  "分层是否清晰"取决于包边界与依赖方向，不取决于模块数量。文档中已给出可拆分边界（common / domain / service / web）。
- **库存采用最终一致**：Redis 预扣 + 定时对账，换取高并发下的吞吐与正确性，
  代价是缓存值可能短暂领先于数据库（接口用 `fromCache` 字段显式标记来源）。
- **前端权限过滤只是体验优化**：真正的权限边界在后端每个接口的 `@RequirePermission` 上，
  前端菜单由"路由表 + 用户权限"推导，避免出现"菜单能点、接口 403"的不一致。
- **深分页没有盲目改成游标分页**：后台管理列表需要页码跳转，保留 OFFSET 但限制最大页深；
  游标分页留给 C 端流式列表（实测快约 570 倍）。
- **自研压测工具而非引入 JMeter**：项目要做到"任何装了 JDK 17 的机器开箱可复现"，
  基于 JDK HttpClient 的 200 行工具足够覆盖本项目场景，且逻辑完全透明。